...
if _HAS_WARP:
    ...
    def _divergence_3d_order2_kernel(
        ...
        kp = _wrap_plus1(k, n2)
        div_x = (vector_field[0, ip, j, k] - vector_field[0, im, j, k]) * (
            0.5 * inv_dx0
        )
        div_y = (vector_field[1, i, jp, k] - vector_field[1, i, jm, k]) * (
            0.5 * inv_dx1
        )
        div_z = (vector_field[2, i, j, kp] - vector_field[2, i, j, km]) * (
            0.5 * inv_dx2
        )
    ...
    def _divergence_backward_2d_order2_kernel(
        ...
        jm = _wrap_minus1(j, n1)
        jp = _wrap_plus1(j, n1)
        grad_vector[0, i, j] = (grad_output[im, j] - grad_output[ip, j]) * (
            0.5 * inv_dx0
        )
        grad_vector[1, i, j] = (grad_output[i, jm] - grad_output[i, jp]) * (
            0.5 * inv_dx1
        )
...
def uniform_grid_divergence_warp(
    ...
    if not _HAS_WARP:
        raise ImportError(
            "uniform_grid_divergence requires the Warp backend; install warp>=0.6.0"
    ...
    return uniform_grid_divergence_impl(vector_field, spacing_meta, int(order))
