...
def test_electric_field_update_torch(device: str):
    ...
    output = electric_field_update(*args, implementation="torch", **kwargs)
    reference = ElectricFieldUpdate.dispatch(*args, implementation="torch", **kwargs)
    ElectricFieldUpdate.compare_forward(output, reference)


@requires_module("warp")
def test_electric_field_update_warp(device: str):
    args, kwargs = _build_case(
        device=device,
        material_mode="field-field",
        use_current=True,
    ...
    output = electric_field_update(*args, implementation="warp", **kwargs)
    assert output.shape == args[0].shape


def test_electric_field_update_make_inputs_forward(device: str):
    label, args, kwargs = next(iter(ElectricFieldUpdate.make_inputs_forward(device)))
    assert isinstance(label, str)
    assert isinstance(args, tuple)
    assert isinstance(kwargs, dict)

    output = ElectricFieldUpdate.dispatch(*args, implementation="torch", **kwargs)
    assert output.shape[0] == 3


def test_electric_field_update_make_inputs_backward(device: str):
    label, args, kwargs = next(iter(ElectricFieldUpdate.make_inputs_backward(device)))
    assert isinstance(label, str)
    assert isinstance(args, tuple)
    assert isinstance(kwargs, dict)
...
@requires_module("warp")
def test_electric_field_update_backend_forward_parity(device: str):
    ...
    ElectricFieldUpdate.compare_forward(out_warp, out_torch)


# Validate warp and torch parity for backward gradients.
@requires_module("warp")
def test_electric_field_update_backend_backward_parity(device: str):
    args_torch, kwargs_torch = _build_case(
        device=device,
        material_mode="field-field",
        use_current=True,
    ...
    ElectricFieldUpdate.compare_backward(electric_warp.grad, electric_torch.grad)

    assert magnetic_warp.grad is not None
    assert magnetic_torch.grad is not None
    ElectricFieldUpdate.compare_backward(magnetic_warp.grad, magnetic_torch.grad)
    ...
    if isinstance(imp_torch, torch.Tensor):
        ...
        assert imp_warp.grad is not None
        assert imp_torch.grad is not None
        ElectricFieldUpdate.compare_backward(imp_warp.grad, imp_torch.grad)
...
def test_electric_field_update_backend_forward_parity_all_variants(
    device: str,
    material_mode: str,
    use_current: bool,
):
...
# Validate warp and torch backward parity for all material/current variants.
@requires_module("warp")
@pytest.mark.parametrize("material_mode", _MATERIAL_MODES)
@pytest.mark.parametrize("use_current", [False, True])
def test_electric_field_update_backend_backward_parity_all_variants(
    device: str,
    material_mode: str,
    use_current: bool,
):
    ...
    if isinstance(eps_torch, torch.Tensor):
        ...
        ElectricFieldUpdate.compare_backward(eps_warp.grad, eps_torch.grad)
    if isinstance(sigma_torch, torch.Tensor):
        assert isinstance(sigma_warp, torch.Tensor)
        assert sigma_warp.grad is not None
        assert sigma_torch.grad is not None
        ElectricFieldUpdate.compare_backward(sigma_warp.grad, sigma_torch.grad)
...
def test_electric_field_update_inplace_matches_out_of_place_all_variants(
    ...
    args, kwargs = clone_case(args, kwargs)
...
def test_electric_field_update_compare_forward_contract(device: str):
    _, args, kwargs = next(iter(ElectricFieldUpdate.make_inputs_forward(device)))
    output = ElectricFieldUpdate.dispatch(*args, implementation="torch", **kwargs)
    reference = output.detach().clone()
    ElectricFieldUpdate.compare_forward(output, reference)


def test_electric_field_update_compare_backward_contract(device: str):
    _, args, kwargs = next(iter(ElectricFieldUpdate.make_inputs_backward(device)))
    electric_field, magnetic_field, _, _, _, _ = args
    ...
    ElectricFieldUpdate.compare_backward(
...
def test_electric_field_update_error_handling(device: str):
    args, kwargs = _build_case(
        device=device,
        material_mode="scalar-scalar",
        use_current=False,
    ...
    invalid_electric = args[0].to(torch.float64)
    with pytest.raises(TypeError, match="electric_field must be float32"):
        ElectricFieldUpdate.dispatch(
            invalid_electric,
            *args[1:],
            implementation="torch",
