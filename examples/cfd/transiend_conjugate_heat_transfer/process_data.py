...
This script reads the raw SimScale dumps, packs each simulation into a single
``.npz`` file, and writes simple scaling stats. Outputs land in
``<processed_dir>/npz`` and ``<processed_dir>/stats``.
