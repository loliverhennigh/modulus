...
@torch.library.custom_op("physicsnemo::pml_phi_h_update_warp", mutates_args=())
def pml_phi_h_update_impl(
...
@pml_phi_h_update_impl.register_fake
def _(
    electric_field: torch.Tensor,
    pml_layer: torch.Tensor,
    offset_x: int,
    ...
    _ = (electric_field, offset_x, offset_y, offset_z)
    return torch.empty_like(pml_layer)


def setup_pml_phi_h_update_context(
    ctx: torch.autograd.function.FunctionCtx,
    inputs: tuple,
    output: torch.Tensor,
) -> None:
    electric_field, pml_layer, offset_x, offset_y, offset_z = inputs
    _ = output
...
def backward_pml_phi_h_update(
    ...
    electric_field, pml_layer = ctx.saved_tensors

    grad_electric, grad_pml = _launch_warp_backward(
        electric_field=electric_field,
        pml_layer=pml_layer,
...
pml_phi_h_update_impl.register_autograd(
    backward_pml_phi_h_update,
    setup_context=setup_pml_phi_h_update_context,
)


def pml_phi_h_update_warp(
    electric_field: torch.Tensor,
    pml_layer: torch.Tensor,
    pml_layer_offset: torch.Tensor | Sequence[int] = (0, 0, 0),
    inplace: bool = False,
) -> torch.Tensor:
    _validate_inputs(electric_field, pml_layer, pml_layer_offset, inplace)
